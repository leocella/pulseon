-- Add 'Agendado' to tipo_atendimento enum
ALTER TYPE public.tipo_atendimento ADD VALUE IF NOT EXISTS 'Agendado';

-- Update next_ticket function to handle 'Agendado' (Prefix 'S')
...
-- Update get_next_to_call to include logic for 'Agendado'
-- Priority: Preferencial (0) > Agendado (1) > Retirada de Laudo (2) > Normal (3)
...
BEGIN
    ...
    ORDER BY 
        CASE 
            ...
            WHEN f.tipo = 'Agendado' THEN 1
            WHEN f.tipo = 'Retirada de Laudo' THEN 2
            ELSE 3 
        END,
        f.hora_emissao ASC
    LIMIT 1;
...
BEGIN
    ...
    SET 
        status = 'chamado', 
        hora_chamada = now(),
        atendente = COALESCE(p_atendente, f.atendente)
